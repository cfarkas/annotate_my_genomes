...
process check_paths {
  ...
  '''
  # mandatory arguments
  if [ ! "!{params.stringtie}" ] || [ ! "!{params.ref_annotation}" ] || [ ! "!{params.genome}" ] || [ ! "!{params.config}" ] || [ ! "!{params.threads}" ] || [ ! "!{params.conda}" ]|| [ ! "!{params.outdir}" ]; then
    echo ""
...
process gffcompare_stringtie_fix {
  ...
  file 'UCSC_compare.stringtie_input_file.gtf.tmap' into tmap_records
  file 'gffcompare_outputs_UCSC' into gffcompare_outputs_UCSC_dir
  ...
  gffcompare -R -r "!{params.ref_annotation}" -s "!{params.genome}" -o UCSC_compare stringtie_input_file.gtf
  ...
  cat UCSC_compare.stringtie_input_file.gtf.tmap | sed "1d" | cut -f4 | sort | uniq | wc -l >> Stats.txt
  echo "" >> Stats.txt
  echo "Number of novel genes:" >> Stats.txt
  cat UCSC_compare.stringtie_input_file.gtf.tmap | awk '$3=="u"{print $0}' | cut -f4 | sort | uniq | wc -l >> Stats.txt
  echo "" >> Stats.txt
  echo "Number of novel transcripts:" >> Stats.txt
  cat UCSC_compare.stringtie_input_file.gtf.tmap | awk '$3=="u"{print $0}' | cut -f5 | sort | uniq | wc -l >> Stats.txt
  echo "" >> Stats.txt
  echo "Number of transcripts matching annotation:" >> Stats.txt
  cat UCSC_compare.stringtie_input_file.gtf.tmap | awk '$3=="="{print $0}' | cut -f5 | sort | uniq | wc -l >> Stats.txt
  exec 3>&-
  echo ""
  ...
  echo ""
  awk '{print $4"\t"$1}' UCSC_compare.stringtie_input_file.gtf.tmap > UCSC_compare.stringtie_input_file.gtf.tmap.1
  tail -n +2 UCSC_compare.stringtie_input_file.gtf.tmap.1 > UCSC_compare.stringtie_input_file.gtf.tmap.2
  awk '$2 != "-"' UCSC_compare.stringtie_input_file.gtf.tmap.2 > namelist
  ...
  cp ./gffcompare_outputs_UCSC/UCSC_compare.stringtie_input_file.gtf.tmap ./
  echo ""
...
process gffread_stringtie {
  ...
  output:
  file 'transcripts.fa' into records2
  ...
  echo ""
  gffread -w transcripts.fa -g "!{params.genome}" final_annotated.gtf
  printf "::: INFO: Number of transcripts:\n"
  grep ">" transcripts.fa -c
  echo ""
...
process gawn_annotation {
  ...
  input:
  file 'transcripts.fa' from records2
...
process output_pipeline {
  ...
  input:
  file 'UCSC_compare.stringtie_input_file.gtf.tmap' from tmap_records
  ...
  if [ -z "$(ls -A "!{params.outdir}")" ]; then
     ...
     mv gffcompare_outputs_UCSC "!{params.outdir}"
     ...
     mv output_files ./annotate_my_genomes_$sec
     mv gffcompare_outputs_UCSC ./annotate_my_genomes_$sec
     mv gawn ./annotate_my_genomes_$sec
     mv feelnc_codpot_out ./annotate_my_genomes_$sec
     mv coding-transcripts.fa.transdecoder_dir ./annotate_my_genomes_$sec
     mv transdecoder ./annotate_my_genomes_$sec
     mv annotate_my_genomes_$sec "!{params.outdir}"
  fi
