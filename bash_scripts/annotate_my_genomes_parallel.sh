...
exec 3<&- 4<&-
        cat merged.annotated.gtf | parallel --pipe sed -f "$tmpfile" > merged.annotated_with_reference.gtf
...
exec 3<&- 4<&-
	cat merged.annotated_with_reference.gtf | parallel --pipe sed -f "$tmpfile" > merged_with_reference.gtf
rm -f "$tmpfile" fileA fileB
echo "Done"
echo ""
